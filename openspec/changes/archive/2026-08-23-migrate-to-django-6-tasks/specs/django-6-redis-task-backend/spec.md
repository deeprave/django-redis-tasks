...
### Requirement: Redis-backed task queue with an embedded worker loop
**Reason**: Superseded by delegating storage and worker execution to `django-queues`.
**Migration**: Configure a `django_queue` `AsyncQueue` (e.g. `RedisAsyncPriorityQueueJson`) via Django's `QUEUES` setting with a `HANDLER` pointing at this package's handler, and run `manage.py runqueues` instead of relying on automatic worker startup in `AppConfig.ready()`.
